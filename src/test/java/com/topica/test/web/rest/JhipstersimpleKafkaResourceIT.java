...
import com.topica.test.config.KafkaProperties;
...
class JhipstersimpleKafkaResourceIT {
    ...
    void setup() {
        ...
        JhipstersimpleKafkaResource kafkaResource = new JhipstersimpleKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhipstersimple-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/jhipstersimple-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
